...
def register_runtime_control_dashboard_routes(
    ...
    async def api_okx_canary_preflight(
        request: Request,
        token: str | None = Query(None),
        x_dashboard_token: str | None = Header(None, alias="X-Dashboard-Token"),
    ):
        ...
        from .okx_canary_preflight import build_okx_canary_preflight, fetch_okx_market_snapshot
            ...
            return JSONResponse(
                content={
                    ...
                    "Preflight": {
                        ...
                        "canary_ready": False,
                        "blocking_reasons": ["not_okx_exchange"],
                    },
                }
            )

        try:
            market = fetch_okx_market_snapshot(trader.exchange_client, DEFAULT_SYMBOL)
            market_error = None
        except Exception as exc:
            from .live_canary_preflight import CanaryMarketSnapshot
        ...
        try:
            report = trader.exchange_client.reconcile(trigger="dashboard_preflight")
        except Exception:
            report = reconcile_service.get_latest_report() if reconcile_service is not None else None

        payload = build_okx_canary_preflight(
            settings,
            runtime_control,
            market=market,
            reconcile_report=report,
